...
try {
	...
	houses.refresh = (id) => {
		...
		container.set(
			...
			mp.labels.new(
				...
				new mp.Vector3(
					container.get('houses', id, 'position').x,
					container.get('houses', id, 'position').y,
					container.get('houses', id, 'position').z + 1
				),
				{
					font: 0,
					drawDistance: 4,
		...
		if (
			parseInt(container.get('houses', id, 'interior').x) !== 0 &&
			parseInt(container.get('houses', id, 'interior').y) !== 0 &&
			parseInt(container.get('houses', id, 'interior').z) !== 0
		) {
			container.set(
				'houses',
				id,
				...
				mp.markers.new(
					1,
					new mp.Vector3(
						parseInt(container.get('houses', id, 'interior').x),
						parseInt(container.get('houses', id, 'interior').y),
						parseInt(container.get('houses', id, 'interior').z)
					),
					1,
					{
		...
		if (
			parseInt(container.get('houses', id, 'garage').position.x) !== 0 &&
			...
			container.set(
				...
				mp.markers.new(
					1,
					new mp.Vector3(
						parseInt(container.get('houses', id, 'garage').house.x),
						parseInt(container.get('houses', id, 'garage').house.y),
						parseInt(container.get('houses', id, 'garage').house.z) - 1.0
					),
					1,
					{
			...
			container
				.set(
					...
					'_colshapeGarage',
					mp.colshapes.newCircle(
						parseInt(container.get('houses', id, 'garage').position.x),
						parseInt(container.get('houses', id, 'garage').position.y),
						4.0,
						container.get('houses', id, 'dimension')
					)
				)
			...
			container.set(
				...
				mp.markers.new(
					...
					new mp.Vector3(
						parseInt(container.get('houses', id, 'garage').position.x),
						parseInt(container.get('houses', id, 'garage').position.y),
						parseInt(container.get('houses', id, 'garage').position.z)
					),
					2.0,
					{
						color: [255, 255, 255, 100],
			...
			container.set(
				...
				'_labelGarage',
				mp.labels.new(
					`Гараж дома #${houses.getID(id)}${houses.getLocked(id) ? '\n\n~r~Закрыт' : ''}`,
					new mp.Vector3(
						parseInt(container.get('houses', id, 'garage').position.x),
						parseInt(container.get('houses', id, 'garage').position.y),
						parseInt(container.get('houses', id, 'garage').position.y) + 1
					),
					{
						font: 0,
						drawDistance: 4,
			...
			container
				.set(
					...
					'_colshapeGarageExit',
					mp.colshapes.newCircle(
						parseInt(container.get('houses', id, 'garage').vehicle.x),
						parseInt(container.get('houses', id, 'garage').vehicle.y),
						4.0,
						houses.getID(id) + 50
					)
				)
			...
			container.set(
				'houses',
				id,
				'_markerCharacterGarage',
				mp.markers.new(
					1,
					new mp.Vector3(
						parseInt(container.get('houses', id, 'garage').character.x),
						parseInt(container.get('houses', id, 'garage').character.y),
						parseInt(container.get('houses', id, 'garage').character.z) - 1.0
					),
					1,
					{
						color: [255, 255, 255, 100],
	...
	houses.getOwner = (id) => {
		if (!houses.isState(id) || container.get('houses', id, 'owner').id < 1) return { id: 0, name: 'Неимеется' };
	...
	houses.create = (type, classes, position, data = {}, callback) => {
		if (type < 0 || type >= enums.housesType.length || classes < 0 || classes >= enums.housesClass.length) return callback(false);

		let randomInterior = func.random(0, enums.housesDefaultSettings[type][classes].interiors.length);
		if (randomInterior >= enums.housesDefaultSettings[type][classes].interiors.length)
			randomInterior = enums.housesDefaultSettings[type][classes].interiors.length - 1;
		...
		if (type == 0) {
			...
			if (randomInteriorGarage >= enums.housesDefaultSettings[type][classes].garages.length)
				randomInteriorGarage = enums.housesDefaultSettings[type][classes].garages.length - 1;
		...
		mysql.query(
			`insert into houses (type, class, position, dimension, interior, price, owner, garage) values (?, ?, ?, ?, ?, ?, '{ "id": 0, "name": "Неимеется" }', ?)`,
			[
				type,
				classes,
				JSON.stringify({
	...
	houses.nearPlayerGarage = (player, id = -1) => {
		...
		if (
			func.distance2D(
				player.position,
				new mp.Vector3(
					container.get('houses', id, 'garage').position.x,
					container.get('houses', id, 'garage').position.y,
					container.get('houses', id, 'garage').position.z
				)
			) > 8.0 ||
			player.dimension !== container.get('houses', id, 'dimension')
		) {
	...
	houses.nearPlayerGarageInterior = (player, id = -1) => {
		...
		if (
			(func.distance2D(
				player.position,
				new mp.Vector3(
					container.get('houses', id, 'garage').vehicle.x,
					container.get('houses', id, 'garage').vehicle.y,
					container.get('houses', id, 'garage').vehicle.z
				)
			) > 8.0 &&
				func.distance2D(
					player.position,
					new mp.Vector3(
						container.get('houses', id, 'garage').character.x,
						container.get('houses', id, 'garage').character.y,
						container.get('houses', id, 'garage').character.z
					)
				) > 2.0 &&
				func.distance2D(
					player.position,
					new mp.Vector3(
						container.get('houses', id, 'garage').house.x,
						container.get('houses', id, 'garage').house.y,
						container.get('houses', id, 'garage').house.z
					)
				) > 2.0) ||
			player.dimension !== houses.getID(id) + 50
		) {
	...
	houses.tp = (player, id) => {
		if (!houses.isState(id) || !user.isLogged(player)) return;
	...
	houses.tpInterior = (player, id, admin = false) => {
		...
		if (
			container.get('houses', id, 'interior').x === 0 &&
	...
	houses.tpGarage = (player, id) => {
		...
		if (player.vehicle) {
			...
			if (houses.nonGarageVehicles(container.get('vehicles', player.vehicle.id, 'model')))
				return user.notify(player, 'Данный транспорт нельзя загнать в гараж', 'error');
			...
			user.setPos(
				...
				container.get('houses', id, 'garage').vehicle.y,
				container.get('houses', id, 'garage').vehicle.z,
				container.get('houses', id, 'garage').vehicle.a,
				houses.getID(id) + 50
			);
		} else
			user.setPos(
				...
				container.get('houses', id, 'garage').character.y,
				container.get('houses', id, 'garage').character.z,
				container.get('houses', id, 'garage').character.a,
				houses.getID(id) + 50
			);
	};
	houses.tpOnGarage = (player, id, vehicle = false) => {
		...
		if (
			container.get('houses', id, 'garage').position.x === 0 &&
		...
		user.setPos(
			...
			container.get('houses', id, 'garage').position.y,
			container.get('houses', id, 'garage').position.z,
			container.get('houses', id, 'garage').position.a,
			container.get('houses', id, 'dimension')
		);
	};

	houses.nonGarageVehicles = (model) => {
		if (!enums.vehiclesData[model] || !enums.vehiclesData[model].type)
			return true[('boat', 'commercial', 'helicopter', 'industrial', 'plane', 'service', 'trailer', 'trains')].forEach((item) => {
	...
	houses.action = (player) => {
		...
		if (houses.nearPlayer(player) !== -1 && houses.isState(houses.nearPlayer(player))) id = houses.nearPlayer(player);
		if (houses.nearPlayerInterior(player) !== -1 && houses.isState(houses.nearPlayerInterior(player))) {
			id = houses.nearPlayerInterior(player);
			interior = true;
		}
		if (houses.nearPlayerGarage(player) !== -1 && houses.isState(houses.nearPlayerGarage(player))) {
			id = houses.nearPlayerGarage(player);
			garage = true;
		}
		if (houses.nearPlayerGarageInterior(player) !== -1 && houses.isState(houses.nearPlayerGarageInterior(player))) {
