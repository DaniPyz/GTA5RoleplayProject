	...
	glob[PROCESS_EVENT] = (player: Player | string, rawData?: string) => {
		...
		if (data.req) {
			...
			switch (environment) {
				...
				case 'cef': {
					ret = (ev) => (mp as any).trigger(PROCESS_EVENT, util.stringifyData(ev));
				}
			}
			if (ret) {
		...
		if (environment === 'client') {
			...
			register(TRIGGER_EVENT_BROWSERS, ([name, args], info) => {
				Object.values(glob.__rpcBrowsers).forEach((browser) => {
					_callBrowser(browser as any, TRIGGER_EVENT, [name, args], {
...
export function register(name: string, cb: ProcedureListener): Function {
	...
	if (environment === 'cef') glob[IDENTIFIER].then((id: string) => (mp as any).trigger(BROWSER_REGISTER, JSON.stringify([id, name])));
...
export function unregister(name: string): void {
	...
	if (environment === 'cef') glob[IDENTIFIER].then((id: string) => (mp as any).trigger(BROWSER_UNREGISTER, JSON.stringify([id, name])));
		...
		case 'cef': {
			...
			return glob[IDENTIFIER].then((browserId: string) => {
				return new Promise((resolve) => {
					...
					(mp as any).trigger(PROCESS_EVENT, util.stringifyData(event));
				});
			});
		}
	}
...
export function triggerBrowsers(player: Player | string, name?: string | any, args?: any) {
	...
	_callClient(player as Player, TRIGGER_EVENT_BROWSERS, [name, args], {
		noRet: 1
	});
}
