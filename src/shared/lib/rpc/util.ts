...
function isObjectMpType(obj: any, type: MpTypes) {
	...
	if (obj && typeof obj === 'object' && typeof obj.id !== 'undefined') {
		...
		switch (type) {
			case MpTypes.Blip:
				return test('blip', mp.blips, (mp as any).Blip);
			case MpTypes.Checkpoint:
				return test('checkpoint', mp.checkpoints, (mp as any).Checkpoint);
			case MpTypes.Colshape:
				return test('colshape', mp.colshapes, (mp as any).Colshape);
			case MpTypes.Label:
				return test('textlabel', mp.labels, (mp as any).TextLabel);
			case MpTypes.Marker:
				return test('marker', mp.markers, (mp as any).Marker);
			case MpTypes.Object:
				return test('object', mp.objects, (mp as any).Object);
			case MpTypes.Pickup:
				return test('pickup', mp.pickups, (mp as any).Pickup);
			case MpTypes.Player:
				return test('player', mp.players, (mp as any).Player);
			case MpTypes.Vehicle:
				return test('vehicle', mp.vehicles, (mp as any).Vehicle);
		}
	}
	return false;
}
...
export function getEnvironment(): string {
	if ((mp as any).joaat) return 'server';
	else if (mp.game && mp.game.joaat) return 'client';
	else if ((mp as any).trigger) return 'cef';
}
