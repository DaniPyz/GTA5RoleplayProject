		...
		<div className="death" data-status={death.status} data-timer={death.timer} style={!death.status ? { display: "none" } : {}}>
			<div className="death-wrap">
				...
				<div className="death-img">
					<img alt="" src={require("./images/death_1.png")} />
					<img alt="" src={require("./images/death_2.png")} />
					<img alt="" src={require("./images/death_3.png")} />
				</div>

				<div className="death-btn">
					<button onClick={() => click(true)} className="btn">
						Вызвать медиков
					</button>
					<button onClick={() => click(false)} className="btn">
						Я хочу на тот свет
					</button>
				</div>
			</div>
