			...
			isOpened && (
				...
				<animated.div style={style} className={s.fraction}>
					<>
						...
						<div className={s.mainLayout}>
							...
							<div className={s.info}>
								...
								<div className={s.info_footer}>
									...
									<h2><p>{8}</p>/<div>{FRACTION_DATA[fractionId].maxWeight} кг</div> <span><WeightVector /></span></h2>
								</div>
									...
									if (state.warehouse && state.warehouse[selected] && state.warehouse[selected][index]) {
										...
										return (
											<section key={index} onClick={() => setSelectedCell(index)}>
												<ItemCell index={index} change={changeSelection} selected={selectedCell} img={state.warehouse[selected][index]!.img} count={state.warehouse[selected][index]!.count} selectedFilter={selected} />
											</section>
										)
									} else {
										return (
						...
						{
							selectedCell !== null ? (<div className={s.infoDialog}>
								<ItemInfo img={state.warehouse[selected][selectedCell]!.img} name={state.warehouse[selected][selectedCell]!.name} weight={state.warehouse[selected][selectedCell]!.weight} />
							</div>) : ('')
						}
